...
export async function POST(request) {
    try {
        ...
        const { user_id, title, description, category, price, monetization_type, text_content, images, thumbnail, // For capturing thumbnail
         } = body;
        ...
        if (images && images.length > 0) {
            art_urls = await Promise.all(images.map(async (filePath) => await uploadImageToCloudinary(filePath)));
        }
...
export async function GET(request) {
    try {
        ...
        const mergedItems = await Promise.all(metadata.map(async (meta) => {
