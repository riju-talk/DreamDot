...
export async function POST(req) {
    try {
        ...
        if (!user_id || !otp) {
            return new Response(JSON.stringify({ error: 'Missing user id or OTP' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        if (!record) {
            return new Response(JSON.stringify({ error: 'No OTP record found for this user' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        if (new Date() > record.otp_expires_at) {
            return new Response(JSON.stringify({ error: 'OTP expired' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
...
export async function PUT(req) {
    try {
        ...
        if (!user_id || !email) {
            return new Response(JSON.stringify({ error: 'Missing user id or email' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
