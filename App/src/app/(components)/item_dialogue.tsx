...
export default function CreateItemModal({
  ...
  const [monetizationType, setMonetizationType] = useState("single");
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (thumbnail) {
        base64Thumbnail = await convertFileToBase64(thumbnail.originFileObj);
      }
  ...
  return (
    <Modal
      ...
      footer={[
        ...
        <Button
          ...
          disabled={!title || !description || !price || (contentType === "art" && fileList.length === 0) || (contentType === "writing" && !writingContent) || !thumbnail}
        >
          Publish
        </Button>,
      ]}
    ...
    >
      <div className="space-y-6">
        ...
        <TextArea rows={3} placeholder="Item Description" value={description} onChange={(e) => setDescription(e.target.value)} />
        
        {contentType === "art" && <Upload.Dragger {...uploadProps}>Upload Artwork</Upload.Dragger>}
        {contentType === "writing" && <TextArea rows={6} value={writingContent} onChange={(e) => setWritingContent(e.target.value)} placeholder="Write your content here..." />}
        
        <Upload.Dragger {...thumbnailUploadProps}>Upload Thumbnail</Upload.Dragger>
        
        <div className="grid grid-cols-2 gap-4">
          <Input prefix={<DollarOutlined />} placeholder="Price" type="number" value={price} onChange={(e) => setPrice(e.target.value)} />
          <Select value={monetizationType} onChange={setMonetizationType} className="w-full">
